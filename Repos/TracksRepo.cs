...
namespace UnchainedBackend.Repos
{
    public interface ITracksRepo
    {
        ...
        Task<IEnumerable<TrackReturn>> GetMyCreated(string OwnerOfPublicAddress);
        ...
        Task<bool>  SetNoBidsFinishedAuction(int Id);
    }
    public class TracksRepo : ITracksRepo
    {
        ...
        {
            return await _context.Tracks.Where(w => w.IsSold == false && (w.IsAuctioned || w.IsListed))
                .Include(t => t.Auction)
                .Include(t=>t.OwnerOf)
                .Include(t=>t.Listing)
                .Select(x => new TrackReturn
            {
                AuctionEnding = x.Auction != null ? (x.Auction.Ending- DateTime.Now.ToUniversalTime()).TotalSeconds : null,
        ...
        public async Task<TrackDetailReturn> GetTrack(int id)
        {
            ...
            TrackDetailReturn trackReturn = new()
            {
                ...
                AuctionEnding = track.Auction != null ? (track.Auction.Ending - DateTime.Now.ToUniversalTime()).TotalSeconds : null,
        ...
        {
            return await _context.Tracks.Where(w => w.IsSold == true && w.OwnerOfPublicAddress == OwnerOfPublicAddress)
               .Include(t => t.Auction)
               .Include(t => t.OwnerOf)
               .Include(t => t.Listing)
               .Select(x => new TrackReturn
        ...
        public async Task<IEnumerable<TrackReturn>> GetMyCreated(string OwnerOfPublicAddress)
        {
            return await _context.Tracks.Where(w => w.IsSold == false && w.OwnerOfPublicAddress == OwnerOfPublicAddress)
               .Include(t => t.Auction)
               .Include(t => t.OwnerOf)
               .Include(t => t.Listing)
               .Select(x => new TrackReturn
        ...
        {
            var track = await _context.Tracks.Include(x=>x.OwnerOf).Include(x => x.Auction).ThenInclude(x=>x.Bids).Include(x=>x.Listing)
                .FirstOrDefaultAsync(x => x.Id == model.TrackId);
            var newUser = await _context.Users.FindAsync(model.To);

            track.IsSold = true;
            track.OwnerOf = newUser;
            track.ListingId = null;
            track.AuctionId = null;
            track.IsAuctioned = false;
            track.IsListed = false;
            if (track.Auction != null)
            {
                _context.RemoveRange(track.Auction.Bids);
                track.Auction.Bids.Clear();
                _context.Auctions.Remove(track.Auction).State = EntityState.Deleted;
            }
            if (track.Listing != null)
            {
                _context.Listings.Remove(track.Listing).State = EntityState.Deleted;
            }
        ...
        {
            Track track = new() { Id = trackId, IsAuctioned = isAuctioned };
        ...
        public async Task<bool> SetNoBidsFinishedAuction(int trackId)
        {
            var track = await _context.Tracks.Include(x => x.OwnerOf).Include(x => x.Auction).ThenInclude(x => x.Bids).Include(x => x.Listing)
